...
#[derive(Clone, Debug, Deserialize, Serialize)]
pub struct TokenCompliantInternal {
    ...
    pub id: u64,
    /// The current owner of this token. Either an account_id or a token_id (if composed). Non-compliant
    pub owner_id: Owner,
    /// Ref:
...
#[derive(Clone, Debug, Deserialize, Serialize)]
pub struct TokenCompliantExternal {
    /// The id of this token on this `Store`. Not unique across `Store`s.
    /// `token_id`s count up from 0. Ref: https://github.com/near/NEPs/discussions/171
    pub id: u64,
    /// The current owner of this token. Either an account_id or a token_id (if composed). String
    pub owner_id: String,
    /// Ref:
    /// https://github.com/near/NEPs/blob/master/specs/Standards/NonFungibleToken/ApprovalManagement.md
    /// Set of accounts that may transfer this token, other than the owner.
